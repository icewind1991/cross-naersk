...
let
  inherit (lib.strings)
    ...
    hasInfix
    ;
  inherit (builtins) removeAttrs map;
  ...
  buildCrossArgs =
    ...
    let
      ...
      rustFlagsWithDeps =
        rustFlags + concatStrings (map (targetDep: " -Clink-arg=-L${targetDep}/lib") targetDeps);
    in
    (recursiveMerge [
      {
        nativeBuildInputs = [
        ...
        "CARGO_TARGET_${targetUpperCase}_LINKER" = "${targetCc}/bin/${targetCc.targetPrefix}cc";
        "AR_${targetUnderscore}" = "${targetCc}/bin/${targetCc.targetPrefix}ar";
        "CC_${targetUnderscore}" = "${targetCc}/bin/${targetCc.targetPrefix}cc";
        "CCX_${targetUnderscore}" = "${targetCc}/bin/${targetCc.targetPrefix}ccx";
        "HOST_CC" = "${hostCc}/bin/${stdenv.cc.targetPrefix}cc";
...
rec {
  buildPackage =
    ...
    let
      ...
      crossArgsForTarget =
        options: if hostTarget != target then (crossArgs options).${target} or { } else { };
      args =
        options:
        {
  ...
  defaultCrossArgsForTargets =
    targets: recursiveMerge (map (target: defaultCrossArgs.${target} or { }) targets);
